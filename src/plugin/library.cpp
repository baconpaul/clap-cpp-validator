...
std::unique_ptr<PluginLibrary> PluginLibrary::load(const std::filesystem::path& path) {
    ...
    if (absolutePath.extension() == ".clap") {
        ...
        if (!executableUrl) {
            throw std::runtime_error("Could not get executable URL within bundle: " + absolutePath.string());
        }
        
        char executablePath[PATH_MAX];
        if (!CFURLGetFileSystemRepresentation(executableUrl, true, 
                                               reinterpret_cast<UInt8*>(executablePath), 
                                               PATH_MAX)) {
...
PluginLibraryMetadata PluginLibrary::metadata() const {
    ...
    for (uint32_t i = 0; i < numPlugins; ++i) {
        ...
        if (!descriptor) {
            throw std::runtime_error("The plugin returned a null plugin descriptor for plugin index " +
                                     std::to_string(i));
        }
        ...
        if (seenIds.count(pluginMeta.id)) {
            throw std::runtime_error("The plugin's factory contains multiple entries for the same plugin ID: " +
                                     pluginMeta.id);
        }
...
std::unique_ptr<Plugin> PluginLibrary::createPlugin(const std::string& id, std::shared_ptr<Host> host) {
    const clap_plugin_factory_t* factory = getPluginFactory();
    if (!factory) {
        throw std::runtime_error("The plugin does not support the plugin factory");
    }
